...
xml_list = os.listdir(ROOT_DIR+DATA_DIR+"VOCdevkit/VOC2007/Annotations")
xml_list.sort()

label_set = set()

for i in range(len(xml_list)):
    xml_path = ROOT_DIR+DATA_DIR+"VOCdevkit/VOC2007/Annotations/"+str(xml_list[i])
...
class Pascal_Voc(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    xml_path = ROOT_DIR+DATA_DIR+"VOCdevkit/VOC2007/Annotations/"+str(xml_list[idx])

    file_name, object_name, bbox = xml_parser(xml_path)
    image_path = ROOT_DIR+DATA_DIR+"VOCdevkit/VOC2007/JPEGImages/"+str(file_name)
...
#
#
writer = SummaryWriter(ROOT_DIR+"Models/Faster_RCNN/runs/Faster_RCNN")
# %load_ext tensorboard
# %tensorboard --logdir=ROOT_DIR+"Models/Faster_RCNN/runs"
...
# len_data = 15000
len_data = 100  # voc2007은 5011개 이미지
term = 10
